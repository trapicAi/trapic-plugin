...
| ツール | 機能 |
|--------|------|
| `trapic-recall` | セッション開始時にプロジェクト知識をロード |
| `trapic-create` | 新しいナレッジトレースを作成 |
| `trapic-search` | キーワード、タグ、scopeで検索 |
| `trapic-update` | トレースのステータス、内容、タグを更新 |
| `trapic-health` | プロジェクト知識の健全性レポート |
| `trapic-decay` | 古い/減衰中の知識をスキャン |
| `trapic-review-stale` | 古いトレースを確認または廃止 |
...
1. **セッション開始** — Hook + CLAUDE.mdが `trapic-recall` をトリガー、フルプロジェクトコンテキストをロード
2. **コーディング中** — 意思決定、規約、事実がトピックタグ付きでサイレントキャプチャ
3. **各決定前** — コンフリクト検出が同じトピックを検索、古いトレースを自動置換
4. **検索** — `trapic-search` が曖昧なクエリからトピックタグを推論、セマンティックマッチング
